      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 max-w-4xl w-full px-4 sm:px-0">
        {data.links.map((item, idx) => (
          <a
            ...
            className={`
              group p-6 bg-white/5 border border-white/10 rounded-3xl
              backdrop-blur-xl transition-all duration-500
              hover:border-fluent-accent/50 hover:bg-white/10
          ...
          >
            <div className="p-3 sm:p-4 bg-fluent-accent/10 rounded-full mb-3 sm:mb-4 group-hover:scale-110 transition-transform">
              <DynamicIcon
                name={item.icon}
                className="w-6 h-6 sm:w-8 sm:h-8 text-fluent-accent"
              />
            </div>
        ...
        <div className="flex items-center justify-center gap-6">
          {data.socials.map((social, idx) => (
            <a
              ...
              aria-label={`Visit Amir Qaribi's ${social.type} profile`}
            >
              <DynamicIcon
                name={social.icon}
