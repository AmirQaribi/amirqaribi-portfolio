...
const SkillsSlideComponent: React.FC<Props> = ({
  isActive,
  data,
}) => {
  return (
    ...
    >
      <div className="mb-8 md:mb-12 text-center max-w-3xl px-4">
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4 cursor-default">
          {data.title}
        </h2>

        <p className="text-gray-400 text-sm sm:text-base md:text-lg cursor-default">
          {data.description}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6 max-w-7xl w-full">
        {data.categories.map((category, catIdx) => (
          <div
            key={category.name}
            className={`
              p-5 sm:p-6 bg-white/5
              border border-white/10 rounded-3xl
              backdrop-blur-xl
              transition-all duration-300
              hover:bg-white/10
              hover:border-fluent-accent/40
          ...
          >
            <h3 className="text-lg sm:text-xl font-semibold mb-4 text-fluent-accent cursor-default">
              {category.name}
            </h3>
            ...
            <div className="space-y-3 sm:space-y-4">
              {category.skills.map((skill) => (
                <div
                  key={skill.name}
                  className="flex items-start gap-3 sm:gap-4 cursor-default"
                >
                  <DynamicIcon
                    name={skill.icon}
                    className="w-5 h-5 sm:w-6 sm:h-6 text-gray-400 mt-1 flex-shrink-0"
                  />
...
export const SkillsSlide = memo(SkillsSlideComponent);