...
export const BackgroundEffects: React.FC<BackgroundEffectsProps> = ({ activeSlide, isMobile }) => {
  return (
    <div className="fixed inset-0 z-0 pointer-events-none transition-all duration-1000 ease-in-out">
      ...
      <div className={`absolute top-[-10%] left-[-10%] w-[40vw] h-[40vw] rounded-full blur-[100px] transition-all duration-1000 opacity-40
        ${activeSlide === 0 && !isMobile ? 'bg-blue-900 translate-x-0 translate-y-0' : ''}
        ${activeSlide === 1 && !isMobile ? 'bg-purple-900 translate-x-20 translate-y-20' : ''}
        ${activeSlide === 2 && !isMobile ? 'bg-cyan-900 translate-x-10 translate-y-40' : ''}
        ${activeSlide === 3 && !isMobile ? 'bg-emerald-900 translate-x-30 translate-y-10' : ''}
        ${isMobile ? 'bg-purple-900 translate-x-0 translate-y-0 opacity-30 w-[80vw] h-[80vw]' : ''}
      `} />

      <div className={`absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] rounded-full blur-[120px] transition-all duration-1000 opacity-30
        ${activeSlide === 0 && !isMobile ? 'bg-gray-800' : ''}
        ${activeSlide === 1 && !isMobile ? 'bg-blue-800 scale-125' : ''}
        ${activeSlide === 2 && !isMobile ? 'bg-indigo-900 scale-100' : ''}
        ${activeSlide === 3 && !isMobile ? 'bg-teal-800 scale-110' : ''}
        ${isMobile ? 'bg-cyan-800 scale-100 opacity-30 w-[90vw] h-[90vw]' : ''}
      `} />
